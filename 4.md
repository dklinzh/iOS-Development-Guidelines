...
当访问一个`CGRect`的`x`，`y`，`width`，`height`时，应该使用\[`CGGeometry`函数\]\[[http:\/\/developer.apple.com\/library\/ios\/\#documentation\/graphicsimaging\/reference\/CGGeometry\/Reference\/reference.html\]代替直接访问结构体成员。苹果的](http://developer.apple.com/library/ios/#documentation/graphicsimaging/reference/CGGeometry/Reference/reference.html]代替直接访问结构体成员。苹果的)`CGGeometry`参考中说到：
...
<
 1,
NYTAdCategoryRealState  = 1 
<
<
 2,
NYTAdCategoryTechnology = 1 
<
<
 3
};
...
* 虽然
  ...
  会被直接解释成
  `NO`
  ，但还是建议在条件判断时保持与nil的比较，因为这样代码更直观。
